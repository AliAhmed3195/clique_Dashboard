...
(function () {
  ...
  function Clique($mdToast, $http, $q, $rootScope, $cookieStore, $location, $window, $cookies, $mdDialog) {
    ...
    service.showSessionExpireDialog = showSessionExpireDialog;
    ...
    function configApp() {
      ...
      if (window.location.href.indexOf("instantinvoice") != -1) {
        document.title = "Instant Invoice";
        configObj.appName = "Instant Invoice";
        ...
        configObj.CPappLogo = "https://app.instantinvoice.com/apps/common/assets/images/logo/CPinstantinvoice.png";
        configObj.qbFinAppLogo = "https://app.instantinvoice.com/apps/common/assets/images/maincircle.jpg";
        configObj.termAndConditions = "https://instantinvoice.com/terms-and-conditions";
        ...
        configObj.termAndConditions = "https://www.clique.center/terms";
    ...
    function showSessionExpireDialog(ev) {
      $mdDialog.show({
        targetEvent: ev,
        template: '<md-dialog>' + '  <md-dialog-content class="md-padding ">&nbsp;<b class="md-title">Session Expired</b><br><p style="color:#000">Your session has expired because of inactivity<br>Press Ok to re-establish your secure session</p></md-dialog-content>' + '  <md-dialog-actions>' + '    <md-button ng-click="closeDialog()" class="md-primary md-hue-3 md-raised">' + '      Ok' + '    </md-button>' + '  </md-dialog-actions>' + '</md-dialog>',
        controller: function ($scope, $mdDialog) {
          var vm = this;
          vm.closeDialog = closeDialog;
          $scope.closeDialog = function () {
            sessionStorage.removeItem('popupInit');
            var device = localStorage.getItem("device");
            if (device == 'mobile') {} else {
              $window.location.assign('/apps/account/login')
            }
            $mdDialog.hide()
          }
        }
    ...
    function callService(method, endPoint, params, appContentType) {
      ...
      switch (method) {
        ...
        case "post":
          ...
          debug.params = params;
          $http.post(apiUrl, params, postParams).then(function (response) {
            deferred.resolve(response);
            debug.data = response.data;
            debug.status = "Success"
          }).catch(function (response) {
